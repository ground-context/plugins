...
public class GroundReadWriteNodeResource {
  ...
  public NodeVersion createNodeVersion(long id, Map<String, Tag> tags, long structureVersionId,
      String reference, Map<String, String> referenceParameters, String name)
      throws GroundException {
    try {
      Node node = this.createNode(name, tags);
      NodeVersion nodeVersion = new NodeVersion(id, tags, structureVersionId, reference,
          referenceParameters, node.getId());
  ...
  public NodeVersion getNodeVersion(long nodeVersionId) throws GroundException {
    GetMethod get =
        new GetMethod(PluginUtil.groundServerAddress + "nodes/versions/" + nodeVersionId);
    try {
  ...
  public Node getNode(String name) throws GroundException {
    HttpMethod get = new GetMethod(PluginUtil.groundServerAddress + "nodes/" + name);
    try {
      String response = PluginUtil.execute(get);
      if (response != null) {
  ...
  public List<Long> getTransitiveClosure(Long nodeVersionId) throws GroundException {
    GetMethod get =
        new GetMethod(PluginUtil.groundServerAddress + "nodes/closure/" + nodeVersionId);
    try {
  ...
  public List<Long> getAdjacentNodes(Long prevVersionId, String edgeName) throws GroundException {
    GetMethod get = new GetMethod(
        PluginUtil.groundServerAddress + "adjacent/" + prevVersionId + "/" + edgeName);
    try {
      return (List<Long>) PluginUtil.getVersionList(get);
    } catch (IOException e) {
      throw new GroundException(e);
  ...
  public Node createNode(String name, Map<String, Tag> tagMap) throws GroundException {
    ...
    try {
      ...
      String encodedUri =
          PluginUtil.groundServerAddress + "nodes/" + URLEncoder.encode(name, "UTF-8");
