...
/**
 * RawStore Implementation using Ground APIs. This is the entry point for Ground RawStore
 * Implementation.
 */
public class GroundStore extends GroundStoreBase {
  ...
  public boolean alterDatabase(String dbname, Database db)
    ...
    try {
      ...
      LOG.error("Alter database failed with: {}", ex.getMessage());
      throw ex;
   ...
   * contains" relationship between T and each attribute. The types of attributes of those nodes,
   * and the fact that A2 and A4 are partition keys would be tags of those nodes. The fact that the
   * table T is in a particular file format (Parquet or Avro) would be a tag on the table node.
   */
  @Override
